...
class devValueCheck(QtCore.QThread):
    ...
    dostop = False  # 是否强制终止
    def __init__(self,inputFileFolderPath,referShpFilePath,outputFilePath):
        """
    ...
    def eachXFileRead(self,eachInputFilePath):
        ...
        signalProcess = '已打开文件  ' + eachInputFilePath
        ...
        try:
            ...
            # 发送信号
            signalProcess = 'X文件读取完成  ' + eachInputFilePath
            self.logRcd.emitInfo(signalProcess)
            print(frameDicList)
            print(materialDicList)

            self.frameDicListAll.extend(frameDicList)
            self.materialDicListAll.extend(materialDicList)


        finally:
            file_object.close()

    def valueCalulate(self):
        """
        所有X文件读取完成后，统一进行偏差值计算
        :return:
        """
        ...
        for tempFrameDic in self.frameDicListAll:
...
if  __name__ == '__main__':
    ...
    referShpFilePath=r'C:\Users\Administrator\Desktop\质检工具\数据文件\武大质检程序样例数据\三维\res.shp'
    outputFilePath=r'C:\Users\Administrator\Desktop\导出x\程序测试\outPut718X.shp'
    test=devValueCheck(inputFileFolderPath,referShpFilePath,outputFilePath)
    test.run()
