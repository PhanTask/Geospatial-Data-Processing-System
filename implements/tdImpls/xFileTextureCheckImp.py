...
class xFileTextureCheck(QtCore.QThread):
    ...
    dostop = False  # 是否强制终止
    def __init__(self,inputFileFolderPath,textureFolder):
        """
        ...
        self.inputFileFolderPath=inputFileFolderPath
        self.textureFolder=textureFolder
        self.FrameList=[]

        self.frameDicListAll = []
        self.materialDicListAll = []

        self.signalProcess=""
        self.inputFilePathList=[]

        self.logRcd = logRcd('贴图完整性检查')
        self.textureNum=0       #已处理模型数
    ...
    def run(self):
        ...
        # try:
            # 获取X文件数量,并得到X文件的列表集合
        self.getmodel3DList()
        for eachInputFilePath in self.inputFilePathList:
            self.eachXFileRead(eachInputFilePath)
            self.modelTableItemDataSignal.emit(self.textureCheck(eachInputFilePath))
            ...
            self.logRcd.emitInfo(eachInputFilePath + "  已检查" + '\n')
            self.logRcd.emitProcessValue(int(100.0 * self.textureNum / self.model3DCount))
    ...
    def getmodel3DList(self):
        """
        获取X文件数量
        :return:
        """
        for dir in os.listdir(self.inputFileFolderPath + '/'):
            if dir[-2:] == '.x' or dir[-2:] == '.X':
                self.model3DCount=self.model3DCount+1
                eachXFilePath = self.inputFileFolderPath + '/' + dir
                self.inputFilePathList.append(eachXFilePath)

    def eachXFileRead(self,eachInputFilePath):
        """
        对每一个X文件进行处理
        :return:
        """
        ...
        file_object = open(eachInputFilePath, 'rU')
        self.signalProcess = '已打开文件  ' + eachInputFilePath
        self.logRcd.emitInfo(self.signalProcess)
        print(self.signalProcess)
        ...
        try:
            ...
            for index in range(len(lineList) - 2):  # 少读末尾，用来标记判断文件尾
                ...
                if isReadFrame and tempLine.find("FrameTransformMatrix") >= 0:
                    ...
                    frameDic['lon'] = frameTransformMatrixList[12]
                    frameDic['lat'] = frameTransformMatrixList[14]
                if isReadFrame and tempLine.find("Mesh  {") >= 0:
                    frameDic['meshName'] = tempLine.split(' ')[1][1:]
                    isReadMesh = True
                if isReadFrame and isReadMesh:
                ...
                if isReadFrame and isReadMeshMaterialList:
                    ...
                    if tempLineNext == '}':
                        ...
                        frameDicList.append(tempFrame)
            # 发送信号
            self.signalProcess = 'X文件读取完成  ' + eachInputFilePath
            self.logRcd.emitInfo(self.signalProcess)
            print(frameDicList)
            print(materialDicList)

            self.frameDicListAll=frameDicList
            self.materialDicListAll=materialDicList
        except Exception as e:
            print(e.__str__())
        finally:
            file_object.close()
    ...
    def textureCheck(self,eachInputFilePath):
        tempLackTextureNameList=[]
        self.signalProcess = self.signalProcess + '.X文件读取完毕，正在检查贴图文件' + '\n'
        for tempMaterialDic in self.materialDicListAll:
            self.textureCount=len(self.materialDicListAll)
            for tempTexture in tempMaterialDic['textureFilename']:
                if not os.path.exists(self.textureFolder +'\\' + tempTexture):
                    tempLackTextureNameList.append((tempTexture,'缺失',
                                                    ...
                                                    eachInputFilePath, self.textureFolder))
                    print("缺失"+tempTexture)
                else:
...
if  __name__ == '__main__':
    inputFileFolderPath = r'C:\Users\Administrator\Desktop\导出x\程序测试'
    #inputFilePath = r'C:\Users\Administrator\Desktop\数据质检\数据文件\三维\testAll709.X'
    textureFolder=r'‪C:\Users\Administrator\Desktop\质检工具\数据文件\武大质检程序样例数据\三维\10\texture - 副本'
    test=xFileTextureCheck(inputFileFolderPath ,textureFolder)
    test.run()