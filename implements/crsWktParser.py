...
class CrsWkt1Parser(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.lexer = lex.lex(module=self, debug=False, optimize=False)
        self.parser = yacc.yacc(module=self, debug=False, write_tables=0, **kwargs)
    ...
    def _lexer_test(self, data):
        ...
        while 1:
            ...
            if not t: break
            print
            "%3d: type: %-15s\tvalue: %s" % (t.lexpos, t.type, t.value)
    ...
    def p_geographic_cs(self, p):
        ...
        if isinstance(p[5], list):
            for node in p[5]:
                ...
                elif node.node_type == 'METADATA':
                    geog_cs.metadata = node
                elif node.node_type == 'UNIT':
    ...
    def p_metadata(self, p):
        """metadata : METADATA LBRACKET NAME ',' l ',' t ',' r ',' b ',' pp ',' uu ',' uc1 ',' uc2 RBRACKET
                    | METADATA LBRACKET NAME ',' l ',' t ',' r ',' b ',' pp ',' uu ',' uc1 ',' uc2 ',' authority RBRACKET"""
        self.logger.debug(str(p[1:]))
        p[0] = CrsWktNode('METADATA', name=p[3], l=p[5], t=p[7], r=p[9], b=p[11], pp = p[13], uu = p[15], uc1 = p[17], uc2 = p[19])
        if len(p) > 21 and p[21] is not None: p[0].authority = p[21]
        self.logger.info("Read METADATA node '%s'" % p[3])
...
def main():
    ...
    if len(sys.argv) < 2:
        print
        "usage: python crswktparser.py wktfile [keyword=value, ...]"
        sys.exit(1)
    wktfile = sys.argv[1]
    kwargs = {}
    if len(sys.argv) > 2:
    ...
    print
    "CS type:", coord_sys.node_type
    print
    "CS name:", coord_sys.name
    print
    "CS as str:\n\t", str(coord_sys)
    print
    "CS as dict:\n\t", coord_sys.as_dict()
...
if __name__ == '__main__':
    ...
    #main()
    import gdal
    ds1 = gdal.Open(r'E:\武大质检程序样例数据\0307\0307\H51G030014_P\H51G030014_P.img')
    crsWktStr1 = ds1.GetProjectionRef()
    parser = CrsWkt1Parser()
    crsWktDict = parser.parse_text(crsWktStr1).as_dict()
    print(crsWktDict)
