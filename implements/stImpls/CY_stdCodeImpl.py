...
import ogr
from implements.stImpls.CY_stdCodeImpl_worker import CY_stdCodeImpl_worker
...
class stdCodeImpl(QtCore.QThread):
    ...
    def __init__(self, indexDirPath, outputDirPath = None):
        ...
        self.workerThread = CY_stdCodeImpl_worker()
    ...
    def run(self, *args, **kwargs):
        ...
        try:
            ...
            self.stdCodeChecker(indexFileNum)
    ...
    def getIndexPathList(self):
        ...
        self.outputPathList = [self.outputDirPath + '/' + dir.replace('.gdb','.xlsx') for dir in os.listdir(self.indexDirPath)]
        return self.indexPathList.__len__()

    # @staticmethod
    # def pystdCodeChecker(indexPath,indexFileNum):
    #     """
    #     执行编码一致性检查操作
    #
    ...
    def stdCodeChecker(self,indexFileNum):
        ...
        for indexPath,outputPath in zip(self.indexPathList,self.outputPathList):
            domPool.apply_async(self.workerThread.pystdCodeChecker,(indexPath, outputPath, indexFileNum),callback=self.pystdCodeCheckerCallBack)
            self.logRcd.emitInfo('[INFO] (' +  indexPath.__str__() + ' 文件已加载，质检结果将批量返回，请稍候...\n')
        domPool.close()
        domPool.join()
